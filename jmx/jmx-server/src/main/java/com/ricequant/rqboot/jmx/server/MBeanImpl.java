...
/**
 * This class was copied from https://github.com/germanescobar/jmx-annotations. Many thanks to German Escobar.
 * <p>
 * This is the DynamicMBean implementation that is returned from the {@link com.ricequant.rqboot.jmx.server
 * .RiceMBeanFactory#createBean(Object)} method.
 *
...
public class MBeanImpl implements DynamicMBean {
  ...
  /**
   * Constructor. Creates an instance using the Object instance that is going to be instrumented and the MBeanInfo that
   * describes the exposed information from the object.
   *
   * @param object
   *         the object that is going to be instrumented.
   * @param mBeanInfo
   *         describes the exposed information of the object.
   */
   ...
   *
   * @param operationName
   *         the name of the operation we are looking for.
   * @param receivedSignature
   *         the signature of the operation we are looking for.
   *
  ...
  private MBeanOperationInfo findMBeanOperation(String operationName, String[] receivedSignature) {
    ...
    for (MBeanOperationInfo mBeanOperation : mBeanOperations) {
      ...
      if (mBeanOperation.getName().equals(operationName) && isAssignableSignature(mBeanOperation.getSignature(),
              receivedSignature)) {
   ...
   *         the MBeanAttributeInfo from which we are trying to find the getter method.
   *
  ...
  private Method findGetterMethod(MBeanAttributeInfo mBeanAttribute) {
    ...
    try {
      return object.getClass().getMethod(prefix + NameHelper.upperCaseFirstLetter(mBeanAttribute.getName()));
    }
  ...
  /**
   * Helper method. Tries to find a setter method from the instrumented object using the <code>mBeanAttribute</code> and
   * the <code>paramType</code>.
   *
   * @param mBeanAttribute
   *         the MBeanAttributeInfo from which we are trying to find the setter method.
   * @param attributeType
  ...
  private Method findSetterMethod(MBeanAttributeInfo mBeanAttribute, Class<?> attributeType) {
    ...
    try {
      return object.getClass()
              .getMethod("set" + NameHelper.upperCaseFirstLetter(mBeanAttribute.getName()), attributeType);
    }
    catch (NoSuchMethodException e) {
      return null;
    }
   ...
   *
   * @param to
   *         the class to which we are assigning.
   * @param from
   *         the class from which we are assigning.
   *
   ...
   *
   * @param primitive
   *         the primitive class for which we are looking the wrapper.
   *
